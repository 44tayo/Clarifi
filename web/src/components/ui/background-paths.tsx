...
export function HeroBackgroundPaths({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        'landing-hero-bg pointer-events-none absolute inset-0 z-0 overflow-hidden',
        className,
      )}
      aria-hidden
    >
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(180deg, transparent 52%, rgba(255,255,255,0.55) 78%, #ffffff 92%)',
        }}
      />
    </div>
  )
      ...
      <div className="relative z-10 container mx-auto px-4 text-center md:px-6">
        ...
        >
          ...
          <div className="group relative inline-block overflow-hidden rounded-2xl bg-gradient-to-b from-white/10 to-black/10 p-px shadow-lg backdrop-blur-lg transition-shadow duration-300 hover:shadow-xl">
            <Button
              variant="ghost"
              className="rounded-[1.15rem] border border-white/15 bg-black/80 px-8 py-6 text-lg font-semibold text-white backdrop-blur-md transition-all duration-300 hover:bg-black/90 hover:shadow-md group-hover:-translate-y-0.5"
            >
