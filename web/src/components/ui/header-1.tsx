    ...
    >
      <nav className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
        <div className="flex min-w-0 items-center gap-1">
          ...
          <div className="rounded-md p-2 hover:bg-accent">{logo ?? <WordmarkIcon className="h-4" />}</div>
        </div>
          ...
          {links.map((link) => (
            <a
              ...
              className={buttonVariants({
                variant: 'ghost',
                className: link.active ? 'bg-accent text-accent-foreground' : undefined,
              })}
              href={link.href}
            >
              {link.label}
      ...
      <MobileMenu open={open} className="flex flex-col justify-between gap-2">
        <div className="grid gap-y-2">
          {links.map((link) => (
            <a
              ...
              className={buttonVariants({
                variant: 'ghost',
                className: cn('justify-start', link.active && 'bg-accent text-accent-foreground'),
              })}
              href={link.href}
        ...
        <div className="flex flex-col gap-2 [&_button]:w-full [&_a]:w-full">{menuActions}</div>
      </MobileMenu>
    </header>
  )
}

/** @deprecated Use `Header1`. */
export const Header = Header1
...
function MobileMenu({ open, children, className, ...props }: MobileMenuProps) {
  ...
  return createPortal(
    <div
      id="mobile-menu"
      className={cn(
        'fixed top-14 right-0 bottom-0 left-0 z-40 flex flex-col overflow-hidden border-y bg-background/95 backdrop-blur-lg supports-[backdrop-filter]:bg-background/50 md:hidden',
      )}
    >
      <div
