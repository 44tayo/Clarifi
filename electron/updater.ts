...
export async function configureUpdater(getWindow: WindowGetter): Promise<void> {
  ...
  autoUpdater.setFeedURL({
    provider: 'github',
    owner: process.env.GH_UPDATE_OWNER?.trim() || '44tayo',
    repo: process.env.GH_UPDATE_REPO?.trim() || 'Clarifi',
  })
  ...
  autoUpdater.checkForUpdates = async () => {
    // GitHub provider often returns a non-absolute feed descriptor from getFeedURL().
    // Only enforce host allowlisting when we have a parseable http(s) URL.
    const feedUrl = autoUpdater.getFeedURL()
    if (typeof feedUrl === 'string' && /^https?:\/\//i.test(feedUrl.trim())) {
      let hostname: string
      try {
        hostname = new URL(feedUrl.trim()).hostname
      } catch {
        throw new Error('Couldn’t check for updates. Invalid update feed URL.')
      }
