...
export function loadAudioPreferences(): AudioPreferences {
  ...
  try {
    ...
    cached = {
      ...
      meetingDetectionMutedBundleIds: Array.isArray(parsed.meetingDetectionMutedBundleIds)
        ? parsed.meetingDetectionMutedBundleIds.filter(
            (id): id is string => typeof id === 'string' && id.trim().length > 0,
          )
        : DEFAULTS.meetingDetectionMutedBundleIds,
