...
function pinWidgetAbove(): void {
  if (!widgetWindow || widgetWindow.isDestroyed()) return
  // Order matters on macOS: all-spaces first, then a high always-on-top level so the
  // HUD follows Spaces and stays above other apps (Jamie/Granola-style floating pill).
  widgetWindow.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true })
  widgetWindow.setAlwaysOnTop(true, 'screen-saver')
  if (process.platform === 'darwin') {
    // Re-assert after level change — otherwise Space switches can drop the pin.
    widgetWindow.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true })
  }
}
...
export function createOrShowWidget(): BrowserWindow {
  if (widgetWindow && !widgetWindow.isDestroyed()) {
    pinWidgetAbove()
    // Keep focus on the user's current app; show without stealing keyboard.
  ...
  // If macOS drops always-on-top after Space / fullscreen transitions, re-pin.
