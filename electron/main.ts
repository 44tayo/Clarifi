...
import { createAppTray, destroyAppTray, syncTrayPermissionState } from './tray'
...
/**
 * Default open size — tall desktop frame (~68% × ~88% of the primary work area),
 * matching the preferred launch window (taller than the short 820×560 frame).
 */
...
function defaultMainWindowBounds(): { x: number; y: number; width: number; height: number } {
  ...
  const width = Math.min(aw, Math.max(720, Math.round(aw * DEFAULT_WINDOW_WIDTH_RATIO)))
  const height = Math.min(ah, Math.max(560, Math.round(ah * DEFAULT_WINDOW_HEIGHT_RATIO)))
...
if (!gotLock) {
  ...
  app.whenReady().then(async () => {
    ...
    // Native menu bar status item (same Clarifi process — not a separate app).
    createAppTray({
      getWindow: () => mainWindow,
      ensureWindow: () => ensureMainWindow(),
    })
    syncTrayPermissionState()
    logStartup('H5', 'tray-created')
  ...
  app.on('window-all-closed', () => {
    // macOS: keep running so meeting detection + tray remain available.
    if (process.platform !== 'darwin') {
      app.quit()
    } else {
      mainWindow = null
