...
describe('widgetBoundsForMode', () => {
  it('keeps top-right anchor when expanding', () => {
    ...
    expect(WIDGET_SIZES.compact.width).toBeGreaterThanOrEqual(140)
    expect(WIDGET_SIZES.compact.height).toBeGreaterThanOrEqual(40)
...
/** Documents the Jamie-style instant-start ordering used by audio:start. */
export const AUDIO_START_UI_ORDER = [
  'markMeetingLive',
  'startRecording',
  'revealWidgetAndHideMain',
  'beginSystemAudioCapture',
  'beginMicDeepgramCapture',
] as const

describe('recording start UI order', () => {
  it('reveals the widget and hides main before slow capture pipelines', () => {
    const widgetStep = AUDIO_START_UI_ORDER.indexOf('revealWidgetAndHideMain')
    const systemStep = AUDIO_START_UI_ORDER.indexOf('beginSystemAudioCapture')
    const deepgramStep = AUDIO_START_UI_ORDER.indexOf('beginMicDeepgramCapture')
    expect(widgetStep).toBeGreaterThan(-1)
    expect(widgetStep).toBeLessThan(systemStep)
    expect(widgetStep).toBeLessThan(deepgramStep)
  })
})
