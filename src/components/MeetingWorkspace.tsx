...
type LiveTab = 'scratchpad' | 'transcript'
...
export function MeetingWorkspace({
  ...
  const [liveTab, setLiveTab] = useState<LiveTab>('scratchpad')
  ...
  return (
    <div className={`meeting-workspace${showAskFloat ? ' has-ask-float' : ''}`}>
      ...
      <div className={`meeting-workspace-main${showLiveLayout && !showEnhanced ? ' is-live' : ''}`}>
        {showEnhanced ? (
          <MeetingDocHeader
            meeting={meeting}
          ...
          <header className="live-meeting-header">
            <div className="live-meeting-heading">
              <input
                className="live-meeting-title"
                value={meeting.title}
                onChange={(event) => onUpdate({ title: event.target.value })}
                aria-label="Meeting title"
              />
              <p className="live-meeting-meta">
                {meeting.startedAt
            ...
            <div className="live-meeting-toolbar">
              <div className="live-meeting-tabs" role="tablist" aria-label="Live meeting panels">
                <button
                  type="button"
                  role="tab"
                  aria-selected={liveTab === 'scratchpad'}
                  className={`live-meeting-tab${liveTab === 'scratchpad' ? ' is-active' : ''}`}
                  onClick={() => setLiveTab('scratchpad')}
                >
                  Scratchpad
                </button>
                <button
                  ...
                  className={`live-meeting-tab${liveTab === 'transcript' ? ' is-active' : ''}`}
              ...
              {recordingState === 'recording' ? (
                <button
                  type="button"
                  className="btn btn-secondary live-meeting-pause"
                  onClick={() => void recording.pause()}
                >
                  Pause
                </button>
              ) : null}
              {recordingState === 'paused' ? (
                <button
                  type="button"
                  className="btn btn-primary live-meeting-pause"
                  onClick={() => void recording.resume()}
                >
                  Resume
                </button>
              ) : null}
              <button
                type="button"
                className="btn btn-danger live-meeting-stop"
                onClick={() => void recording.stop()}
              >
                <span className="live-meeting-stop-icon" aria-hidden />
                Stop
              </button>
            </div>
          </header>
