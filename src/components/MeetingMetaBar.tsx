  ...
  if (!documentLayout) {
    return (
      <div className="meeting-meta-bar">
        ...
        <div className="meta-pill-wrap meeting-meta-folders">
        {selectedFolders.map((folder) => (
          <span key={folder.id} className="meeting-folder-chip">
            <button
              type="button"
              className="meeting-folder-chip-main"
              onClick={() => {
                setFolderOpen((v) => !v)
                setSpeakersOpen(false)
              }}
            >
              <FolderGlyph icon={folder.icon} color={folder.color} size={13} />
        ...
        {selectedFolders.length === 0 ? (
          ...
          <button
            type="button"
            className="meta-pill meeting-folder-add"
            aria-label="Add to another folder"
            onClick={() => {
              setFolderOpen((v) => !v)
              setSpeakersOpen(false)
            }}
    ...
    <div className="meeting-meta-bar meeting-meta-bar-doc">
      ...
      <div className="meta-pill-wrap meeting-meta-folders">
        ...
        {selectedFolders.length === 0 ? (
          ...
          >
            <FolderGlyph icon="folder" color="gray" size={13} />
            Add to folder
          </button>
        ) : (
          <button
